...
alter table public.player_migration_intents
  add column account_auth_user_id uuid references auth.users(id) on delete restrict;

create index migration_intents_account_idx
  on public.player_migration_intents (account_auth_user_id, created_at desc);

create or replace function private.migration_profile_summary(target_player_id uuid)
...
revoke all on function private.migration_profile_summary(uuid) from public, anon, authenticated;
