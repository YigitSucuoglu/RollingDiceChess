...
PROFILE-IDENTITY-01A adds a public discriminator without changing authentication identity. PROFILE-IDENTITY-01B adds a single application-level gate after auth/profile initialization: an authenticated canonical profile with `username_onboarding_required` cannot enter normal routes until the caller-authorized rename RPC completes. The gate has no local bypass flag and exposes Sign Out, not Skip. Rename still resolves the owned UUID PlayerId from `auth.uid()`; neither UUID nor discriminator is a browser mutation target.
